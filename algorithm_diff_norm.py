...
__all__ = [
    "DiffNormServer",
    "DiffNormClient",
    "DiffNormServerConfig",
    "DiffNormClientConfig",
]


@register_algorithm()
class DiffNormServerConfig(BaseServerConfig):
    """Server config for the DiffNorm algorithm.
    ...
    norm_ord : str or int or float, default 2
        The norm to use to calculate the distance.
    ...
    **kwargs : dict, optional
    ...
    def __init__(
        ...
        norm_ord: Union[str, int, float] = 2,
...
@register_algorithm()
class DiffNormClientConfig(BaseClientConfig):
    """Client config for the DiffNorm algorithm.
...
@register_algorithm()
@add_docstring(BaseServer.__doc__.replace(_base_algorithm, "DiffNorm"))
class DiffNormServer(BaseServer):

    __name__ = "DiffNormServer"
    ...
    def client_cls(self) -> type:
        return DiffNormClient

    @property
    def config_cls(self) -> Dict[str, type]:
        return {
            "server": DiffNormServerConfig,
            "client": DiffNormClientConfig,
        }

    @property
    def required_config_fields(self) -> List[str]:
        return ["num_clusters", "num_warmup_iters", "local_warmup", "norm_ord"]

    def communicate(self, target: "DiffNormClient") -> None:
    ...
    def _perform_clustering(self) -> None:
        ...
        with tqdm(
            range(self.config.num_clients - 1),
            total=self.config.num_clients - 1,
...
@register_algorithm()
@add_docstring(BaseClient.__doc__.replace(_base_algorithm, "DiffNorm"))
class DiffNormClient(BaseClient):

    __name__ = "DiffNormClient"
    ...
    def communicate(self, target: "DiffNormServer") -> None:
