...
@register_algorithm()
class GradCosServerConfig(BaseServerConfig):
    """Server config for the GradCos algorithm.
...
@register_algorithm()
class GradCosClientConfig(BaseClientConfig):
    """Client config for the GradCos algorithm.
...
@register_algorithm()
@add_docstring(BaseServer.__doc__.replace(_base_algorithm, "GradCos"))
class GradCosServer(BaseServer):

    __name__ = "GradCosServer"
    ...
    def client_cls(self) -> type:
        return GradCosClient

    @property
    def config_cls(self) -> Dict[str, type]:
        return {
            "server": GradCosServerConfig,
            "client": GradCosClientConfig,
        }
    ...
    def communicate(self, target: "GradCosClient") -> None:
    ...
    def _perform_clustering(self) -> None:
        ...
        with tqdm(
            ...
            for client_id in pbar:
                client = self._clients[client_id]
                # client.model.eval()
                client_grad = client.get_gradients()
                # flatten the gradients
                client_grad = torch.cat([g.view(-1) for g in client_grad], dim=0)
...
@register_algorithm()
@add_docstring(BaseClient.__doc__.replace(_base_algorithm, "GradCos"))
class GradCosClient(BaseClient):

    __name__ = "GradCosClient"
    ...
    def communicate(self, target: "GradCosServer") -> None:
